...
class MMU_MRU(MMU):
    ...
    def unload_page(self, ptr_id):
        pages_to_unload = [p for p in self.recently_used if p.get_ptr_id() != ptr_id]
        page_to_unload = pages_to_unload.pop(0)
        self.recently_used.remove(page_to_unload)
    ...
    def new(self, pid, size):
        ...
        for i in range(len(pages)):
            ...
            if self.RAM.available_ram >= page_size:
                ...
                recently.append(page_load)  # SE AGREGA A LISTA TEMPORAL
        ...
        [self.recently_used.insert(0, p) for p in recently] # SE AGREGA A LA PÁGINA USADA MÁS RECIENTE
    ...
    def use(self, ptr_id):
        ...
        for page_id in pages_in_ptr:
            if not page_id in pages_loaded:  # FAILURE
                ...
                self.recently_used.insert(0, page_load)  # SE AGREGA A LA PÁGINA USADA MÁS RECIENTE
                self.add_page_to_memory_table(page_load.page_id,
                                              page_load)  # SE ACTUALIZA DE LA TABLA DE MEMORIA

            else:                            # HIT
                page_used = [p for p in self.RAM.memory if page_id == p.get_page_id()][0] # SE OBTIENE LA PÁGINA EN USO
                self.recently_used.remove(page_used)
                self.recently_used.insert(0, page_used)
    ...
    def delete(self, ptr_id):
        ...
        for p in pages_in_ptr:
            if p in pages_loaded:
                ...
                if size < self.RAM.page_size:
                    self.decrement_fragmentation(self.RAM.page_size - size)
                # SE ELIMINA DE LAS MÁS RECIENTE USADAS
                self.recently_used.remove(page_unloaded)
            else:
